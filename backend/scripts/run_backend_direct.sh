...
if [ "$(docker ps -aq -f name=^test_ting_backend$)" ]; then
    echo "停止现有容器..."
    docker stop test_ting_backend 2>/dev/null
    docker rm test_ting_backend 2>/dev/null
fi
...
# 运行容器 - 使用bridge网络而不是container网络，因为需要端口映射
echo "启动Ting后端服务..."
docker run -d \
    --name ting_backend \
    --network bridge \
...
if curl -f http://localhost:8001/health > /dev/null 2>&1; then
    ...
    echo "服务状态: docker ps | grep ting_backend"
else
    echo "✗ 服务启动失败，查看日志:"
    docker logs ting_backend
    echo "检查容器状态: docker ps -a | grep ting_backend"
fi